...
//        setImageBitmap(bitmap, 0);
        setImageBitmap(bitmap, RotateBitmap.INITIAL_ROTATION);
    }
    ...
    //该方法进来两次，裁剪前进来确定图片方向，裁剪后进来确定要不要旋转。。
    //由于裁剪后图片都是正方形的，所以不用考虑旋转。。只要沿用第一次进来确定图片方向的度数就可以。
    //如果以后裁剪图片不是正方形的话，再考虑。
    public void setImageBitmapResetBase(final Bitmap bitmap,
    	...
    	LogUtils.loge(TAG, "h - " + height+" , w - " + width);
    	if(height> width){
    		RotateBitmap.setInitialRotation(0);
    	}else if(height< width){
    		RotateBitmap.setInitialRotation(90); //默认顺时针转90度
    	}else{
    		//等于的时候，就不用旋转,也不用设定，沿用开始的
    	}
    	
    	}
    	}else{
    		RotateBitmap.setInitialRotation(0); //非拍照，重置为0
    	}
