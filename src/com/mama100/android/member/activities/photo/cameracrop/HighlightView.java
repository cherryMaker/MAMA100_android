...
class HighlightView {
    ...
    //当图片方向性改变的时候，响应的移动方向也要改变
    void moveBy(float dx, float dy) {
    	
    	//added by edwar. 2012-09-07  02:26
    	
    	if(!BasicApplication.getInstance().isFromGallery()){
		if (RotateBitmap.INITIAL_ROTATION % 360 == 90) {
			float temp = dy;
			dy = -dx;
			dx = temp;
		} else if (RotateBitmap.INITIAL_ROTATION % 360 == 180) {
			dx = -dx;
			dy = -dy;
		} else if (RotateBitmap.INITIAL_ROTATION % 360 == 270) {
			float temp = dx;
			dx = -dy;
			dy = temp;
		}
