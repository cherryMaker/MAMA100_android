...
public class HorizontalPager extends ViewGroup {
    ...
    public static final String TAG = HorizontalPager.class.getSimpleName();
    ...
    //add by eco
    private YScrollingListener mYScrollingLis=null;
    /**
    ...
    public HorizontalPager(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.com_mama100_android_member_ui_HorizontalPager);
        pageWidthSpec = a.getDimensionPixelSize(R.styleable.com_mama100_android_member_ui_HorizontalPager_pageWidth, SPEC_UNDEFINED);
        a.recycle();
    ...
    private void checkStartScroll(float x, float y) {
        ...
        if (xMoved || yMoved) {
        	...
        	//Don't react with Y scrolling when mYScrollingLis is null,edited by eco
        	if(mYScrollingLis==null){
                if (xMoved) {
	            ...
	            if (xMoved&&xDiff>yDiff) {
	                // Scroll if the user moved far enough along the X axis and farther than Y axis
	                mTouchState = TOUCH_STATE_SCROLLING;
	                enableChildrenCache();
	            }
	            //react with Y scrolling motion
	            else{
	            	mYScrollingLis.OnYScrolling((int) (mLastMotionY-y));
	            }
        	}
            ...
            if (mAllowLongPress) {
                ...
                if (currentScreen != null) { // add by jimmy.
    ...
    /**
     * call listener if the user moved along y axis farther than X axis
     * @param listener
     */
    public void setYScrollingListener(YScrollingListener listener){
    	mYScrollingLis=listener;
    }
    
    //add by eco
    public interface YScrollingListener{
    	/**
    	 * 
    	 * @param yDistance y distance,positive or negative
    	 */
